...
import { updateCssModule, updateJSModule } from './utils/updateModuleGraph';
...
export default function vitePluginSvelte(inlineOptions?: Partial<Options>): Plugin[] {
	...
	return [
		...
		{
			name: 'vite-plugin-svelte:moduleGraphUpdater',
			apply: 'serve',
			enforce: 'post',
			async transform(code, id, ssr) {
				if (!options.server) {
				...
				log.debug('transform:moduleGraphUpdater', svelteRequest);
				const { query } = svelteRequest;
				const cachedCompileData = cache.getCompileData(svelteRequest, true);
				// unfortunately this still runs before vite own importAnalysis plugin
				// https://github.com/vitejs/vite/blob/2a89c57d3fb2dedbe595c4d49c454f3d138e6414/packages/vite/src/node/plugins/index.ts#L66
				if (query.svelte) {
					// tagged svelte request, use cache
					if (query.type === 'style') {
						await updateCssModule(cachedCompileData!, options.server.moduleGraph, this);
					}
				} else {
					await updateJSModule(cachedCompileData!, options.server.moduleGraph, this);
				}
